...
#####################################################
#           QuadTrees grid compression              #
#####################################################


class QuadTreePositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, visualize=True):
        ...
        # Initializing the board - creating the quadtree
        self.boundary = Rectangle(1, 1, self.walls.height - 2, self.walls.width - 2)
        self.qtree = QuadTree(boundary, 0)
        for (x, y) in self.walls:
            p = Point(x, y, True)
            self.qtree.insert(p)
        for x in range(0, self.qtree.boundary.width):
            for y in range(0, self.qtree.boundary.height):
                if (x, y) not in walls:
                    p = Point(x, y, False)
                    self.qtree.insert(p)
        ...
        self.goal = goal
        self.goalTree = self.qtree.search(encode_coord(goal[0], goal[1]))
    ...
    def isGoalState(self, state):
        stateTree = self.qtree.search(encode_coord(state[0], state[1]))
        isGoal = stateTree == self.goalTree
    ...
    def getSuccessors(self, state):
        ...
        neighbours = []
        morton_key = encode_coords(state[0], state[1])
        current = self.qtree.search(morton_key)
        self.qtree.find_neighbours(morton_key, neighbours)

        successors = []
        (x, y, width, height) = current.boundary
        for next_n in neighbours:
            (nx, ny, nwidth, nheight) = next_n.boundary
            if x + width == nx:
                ...
                action = Directiosn.STOP
            cost = self.costFn((nx, ny))
...
class QuadTreeSearchAgent(SearchAgent):

    def __init__(self):
        self.searchFunction = lambda prob: search.aStarQuadTreeSearch(prob, manhattanHeuristic)
        self.searchType = QuadTreePositionSearchProblem
...
def cornersHeuristic(state, problem):
    ...
    explored = state[1]    # Visited corners
    ...
    # manhattanDistance can never be negative.
    while unvisited:
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        frontier = util.Queue()
        explored = []  # List of already visited nodes
        ...
        while not frontier.isEmpty():
            ...
            if node not in explored:
                ...
                for successor in problem.getSuccessors(node):
                    ...
                    frontier.push((coordinate, actions + [direction]))
